 ...
 * This code performs isotropic segmentation using the "Split Bregman" 
 * algorithm.
 ...
 *   - "mu" is the weighting parameter for the fidelity term (mu should be 
 *        about 1e-4 for images with pixels on the 0-255 scale)
 *   - "edge" is a 2D array of edge detector values at each pixel (e.g. the 
 *        weights for TV regularizer).  For standard segmentation simply 
 *        choose "edge = ones(size(f))"
 *
 */
...
double sbseg(double** f, double** edge,  double** u, double** x,double** y, 
        double** bx, double** by, double *c1, double *c2, double mu, 
        double lambda, int nIter, int width, int height);


/*  Methods for computing averages of each region*/
void updateC(double** f, double** u, double thresh, double *c1, double *c2, 
        int rows, int cols);
void guessC(double** f, double** u, double *c1, double *c2, int rows, 
        int cols);


/******************Relaxation Methods*****************/
double gsUseg(double** f, double** u, double** x, double** y, double** bx, 
        double** by, double c1, double c2, double mu, double lambda, 
        int rows, int cols);
void gsSpaceseg(num** u, num** edge, num** x, num** y, num** bx, num** by, 
...
} /* end function gsSpaceseg */

/* function bregmanXseg */
void bregmanXseg(num** x,num** u, num** bx, int width, int height) {
  int w,h;
  double d;
  num* uwp1,*uw,*bxw,*xw;
