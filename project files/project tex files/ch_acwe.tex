...
As was mentioned, the numerical scheme we apply to \eqref{eq:acwe_el} will largely be the same as the scheme detailed for the GAC model in \Cref{sec:gac_num_disc}. The difference are $\delta_\varepsilon(\phi^n_{ij})$ in place of $\abs{\nabla^0 \phi^n}$, $g_{ij} = 1$ for all $(i,j)$, and the inclusion of the new fitting terms.
...
\right),
\end{align*}
with $a_1, a_2, a_3, a_4$  as given in \Cref{sec:gac_num_disc} but with $g_{ij} = 1$, and $a_0 = 1 + \Delta t \delta_h(\phi_{ij}^n)(a_1 + a_2 + a_3 + a_4)$.
